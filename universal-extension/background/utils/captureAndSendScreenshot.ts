...
export async function captureAndSendScreenshot(
  ...
  try {
    ...
    console.log(
      `Background: Found target window ${targetWindow.id} for screenshot`
    )

    // Capture screenshot of the window containing the tab
    console.log(
      `Background: Capturing screenshot of tab ${message.tabId} in window ${targetWindow.id}`
    )
    ...
    if (chrome.runtime.lastError) {
      ...
      if (chrome.runtime.lastError.message.includes("devtools://")) {
        console.warn(
          "Background: DevTools panel capture error (expected):",
    ...
    try {
      ...
      console.log(
        "Background: Screenshot saved successfully on server:",
        result.path
      )
      ...
      console.error(
        "Background: Error sending screenshot to server directly:",
        uploadError
      )
    ...
    if (!serverUploadSuccess) {
      console.log(
        "Background: Falling back to DevTools panel for screenshot delivery"
      )
