...
let devtoolsSettings = { ...defaultSettings };
...
chrome.storage.local.get(["browserConnectorSettings"], (result) => {
  if (result.browserConnectorSettings) {
    devtoolsSettings = { ...devtoolsSettings, ...result.browserConnectorSettings };
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "SETTINGS_UPDATED") {
    ...
    if (
      ws &&
      (message.settings.serverHost !== devtoolsSettings.serverHost ||
        message.settings.serverPort !== devtoolsSettings.serverPort)
    ) {
        ...
        if (message.type === "heartbeat-response") {
          ...
          chrome.tabs.captureVisibleTab(null, { format: "png" }, (dataUrl) => {
            if (chrome.runtime.lastError) {
              ...
              if (ws && ws.readyState === WebSocket.OPEN) {
                ...
                console.error("Chrome Extension: WebSocket not open to send screenshot error");
              }
            ...
            const response = {
              ...
              ...(devtoolsSettings.screenshotPath && { path: devtoolsSettings.screenshotPath }),
          ...
          chrome.runtime.sendMessage(
            ...
            (response) => {
              ...
              if (response && response.url) {
                console.log(
                  "Chrome Extension: Got URL from background for response:",
                  response.url
                );
                ws.send(
                  JSON.stringify({
