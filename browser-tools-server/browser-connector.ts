...
import { runPerformanceAudit, runAccessibilityAudit, runSEOAudit, AuditCategory, LighthouseReport } from "./lighthouse/index.js"
    ...
    if (inputPath.startsWith("\\\\") || inputPath.includes("\\")) {
      ...
      if (inputPath.includes("wsl.localhost") || inputPath.includes("wsl$")) {
        ...
        const wslIndex = parts.findIndex(
          (part) =>
            part === "wsl.localhost" || part === "wsl$" || part.toLowerCase() === "wsl.localhost" || part.toLowerCase() === "wsl$",
        )
...
export class BrowserConnector {
    ...
    this.wss.on("connection", (ws: WebSocket) => {
      ...
      ws.on("message", (message: string | Buffer | ArrayBuffer | Buffer[]) => {
        try {
          ...
          } else if (data.type === "heartbeat" || data.type === "hello") {
            console.log("Received heartbeat/hello from extension")
          } else {
  ...
  async captureScreenshot(req: express.Request, res: express.Response) {
    ...
    if (!this.activeConnection) {
      console.log("Browser Connector: No active WebSocket connection to Extension")
      return res.status(503).json({ error: "Extension not connected" })
    }
    ...
    try {
      ...
      // Always prioritize the path from the Extension
   ...
   */
  private setupAuditEndpoint(auditType: string, endpoint: string, auditFunction: (url: string) => Promise<LighthouseReport>) {
